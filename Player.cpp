...
bool Player::move(SDL_Event event, const Uint8 *state, int curr_win_width, int curr_win_height, SDL_Rect map_pos, int Key_Reverse)
{
    ...
    {
        if (map_data[(position.y - map_pos.y + (surface->h) / 2) * MAP_FRAME_WIDTH + position.x - map_pos.x + Key_Reverse + (surface->w) / 2] != 1)
        {
        ...
        if (map_data[(position.y - map_pos.y - Key_Reverse + (surface->h) / 2) * MAP_FRAME_WIDTH + position.x - map_pos.x + (surface->w) / 2] != 1)
        {
    ...
    case SDL_KEYDOWN:
        ...
        case SDLK_ESCAPE :
            ...
            break;
        case SDLK_UP:
            if (map_data[(position.y - map_pos.y - Key_Reverse + (surface->h) / 2) * MAP_FRAME_WIDTH + position.x - map_pos.x + (surface->w) / 2] != 1 && map_data[(position.y - map_pos.y - 2 + (surface->h) / 2) * MAP_FRAME_WIDTH + position.x - map_pos.x + (surface->w) / 2] != 1)
            {
                if (position.y > map_pos.y)
                {
                    position.y -= Key_Reverse;
            ...
            break;
        case SDLK_LEFT:
            if (map_data[(position.y - map_pos.y + (surface->h) / 2) * MAP_FRAME_WIDTH + position.x - map_pos.x - Key_Reverse + (surface->w) / 2] != 1 && map_data[(position.y - map_pos.y + (surface->h) / 2) * MAP_FRAME_WIDTH + position.x - map_pos.x - 2 + (surface->w) / 2] != 1)
            {
                if (position.x > map_pos.x)
                {
                    position.x -= Key_Reverse;
